...
namespace Engine.Input.Data
{
	public class MouseData : InputData
	{
		...
		internal InputStates[] Buttons
		{
			set => buttons = value;
		}

		// TODO: If high-DPI settings are used, should probably change these to regular vec2.
		public ivec2 Location { get; }
		public ivec2 PreviousLocation { get; }
		public ivec2 Delta => Location - PreviousLocation;

		public override InputStates this[int data]
		{
			get
			{
				Debug.Assert(data >= 0 && data <= GLFW_MOUSE_BUTTON_LAST, "Invalid mouse button (outside maximum " +
					"range).");
		...
		public override bool Query(int data, InputStates state)
		{
			...
			return InputUtilities.Query(buttons[data], state);
		}
	}
}
