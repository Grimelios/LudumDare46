...
namespace Engine.Physics._3D
{
    ...
    public interface IPoseIntegratorCallbacks
    {
        ...
        /// <summary>
        /// Called prior to integrating the simulation's active bodies3D. When used with a substepping timestepper, this could be called multiple times per frame with different time step values.
        /// </summary>
    ...
    public static class PoseIntegration
    {
        ...
        public static void RotateInverseInertia(in Symmetric3x3 localInverseInertiaTensor, in Quaternion orientation, out Symmetric3x3 rotatedInverseInertiaTensor)
        {
            ...
            //This would be totally fine for all the primitive types which happen to have diagonal inertias, but for more complex physicsShapes3D (convex hulls, meshes), 
    ...
    /// <summary>
    /// Integrates the velocity of mobile bodies3D over time into changes in position and orientation. Also applies gravitational acceleration to dynamic bodies3D.
    /// </summary>
    /// <remarks>
    ...
    /// This integrator also assumes that the bodies3D positions are stored in terms of single precision floats. Later on, we will likely modify the Bodies3D
    ...
    public class PoseIntegrator<TCallbacks> : IPoseIntegrator where TCallbacks : IPoseIntegratorCallbacks
    {
        ...
        public PoseIntegrator(Bodies3D bodies3D, PhysicsShapes3D shapes, BroadPhase broadPhase, TCallbacks callback)
        {
        ...
        void IntegrateAngularVelocity(in Quaternion previousOrientation, in RigidPose pose, in BodyInertia localInertia, in BodyInertia inertia, ref Vector3 angularVelocity, float dt)
        {
            ...
            {
                if (!Bodies3D.HasLockedInertia(localInertia.InverseInertiaTensor))
                {
        ...
        {
            ref var basePoses = ref bodies3D.ActiveSet.Poses[0];
            ref var baseVelocities = ref bodies3D.ActiveSet.Velocities[0];
            ref var baseLocalInertia = ref bodies3D.ActiveSet.LocalInertias[0];
            ref var baseInertias = ref bodies3D.Inertias[0];
            ref var baseActivity = ref bodies3D.ActiveSet.Activity[0];
            ref var baseCollidable = ref bodies3D.ActiveSet.Collidables[0];
        ...
        {
            var boundingBoxUpdater = new BoundingBoxBatcher(bodies3D, shapes, broadPhase, threadDispatcher.GetThreadMemoryPool(workerIndex), cachedDt);
            var bodyCount = bodies3D.ActiveSet.Count;
        ...
        void PrepareForMultithreadedExecution(float dt, int workerCount)
        {
            ...
            bodiesPerJob = bodies3D.ActiveSet.Count / targetJobCount;
            if (bodiesPerJob == 0)
                bodiesPerJob = 1;
            availableJobCount = bodies3D.ActiveSet.Count / bodiesPerJob;
            if (bodiesPerJob * availableJobCount < bodies3D.ActiveSet.Count)
                ++availableJobCount;
        }
        ...
        public void IntegrateBodiesAndUpdateBoundingBoxes(float dt, BufferPool pool, IThreadDispatcher threadDispatcher = null)
        {
            ...
            bodies3D.EnsureInertiasCapacity(Math.Max(1, bodies3D.ActiveSet.Count));
            ...
            if (threadDispatcher != null)
            {
                ...
                //Note that this bottleneck means the fact that we're working through bodies3D in a nonvectorized fashion (in favor of optimizing storage for solver access) is not a problem.
            ...
            {
                var boundingBoxUpdater = new BoundingBoxBatcher(bodies3D, shapes, broadPhase, pool, dt);
                IntegrateBodiesAndUpdateBoundingBoxes(0, bodies3D.ActiveSet.Count, dt, ref boundingBoxUpdater, 0);
                boundingBoxUpdater.Flush();
            }
        }
        ...
        public void PredictBoundingBoxes(float dt, BufferPool pool, IThreadDispatcher threadDispatcher = null)
        {
            ...
            {
                var boundingBoxUpdater = new BoundingBoxBatcher(bodies3D, shapes, broadPhase, pool, dt);
                PredictBoundingBoxes(0, bodies3D.ActiveSet.Count, dt, ref boundingBoxUpdater, 0);
                boundingBoxUpdater.Flush();
            }
        ...
        public void IntegrateVelocitiesBoundsAndInertias(float dt, BufferPool pool, IThreadDispatcher threadDispatcher = null)
        {
            ...
            {
                var boundingBoxUpdater = new BoundingBoxBatcher(bodies3D, shapes, broadPhase, pool, dt);
                IntegrateVelocitiesBoundsAndInertias(0, bodies3D.ActiveSet.Count, dt, ref boundingBoxUpdater, 0);
                boundingBoxUpdater.Flush();
            }
        }
        ...
        public void IntegrateVelocitiesAndUpdateInertias(float dt, BufferPool pool, IThreadDispatcher threadDispatcher = null)
        {
            ...
            {
                IntegrateVelocities(0, bodies3D.ActiveSet.Count, dt, 0);
            }
        }
        ...
        public void IntegratePoses(float dt, BufferPool pool, IThreadDispatcher threadDispatcher = null)
        {
            ...
            {
                IntegratePoses(0, bodies3D.ActiveSet.Count, dt, 0);
            }
        }
    }
}
