...
namespace Engine.Physics._3D
{
    public class IslandSleeper
    {
        ...
        /// <summary>
        /// Gets or sets the fraction of the active set to target as the number of bodies3D slept in a given frame.
        /// This is only a goal; the actual number of slept bodies3D may be more or less.
        /// </summary>
        public float TargetSleptFraction { get; set; } = 0.005f;
        /// <summary>
        /// Gets or sets the fraction of the active set to target as the number of bodies3D traversed for sleeping in a given frame.
        /// This is only a goal; the actual number of traversed bodies3D may be more or less.
        /// </summary>
        public float TargetTraversedFraction { get; set; } = 0.01f;

        public IslandSleeper(Bodies3D bodies3D, Solver solver, BroadPhase broadPhase, ConstraintRemover constraintRemover, BufferPool pool)
        {
        ...
        struct SleepPredicate : IPredicate<int>
        {
            ...
            public bool Matches(ref int bodyIndex)
            {
                return bodies3D.ActiveSet.Activity[bodyIndex].SleepCandidate;
            }
        }
            ...
            BufferPool pool, ref TTraversalPredicate predicate) where TTraversalPredicate : IPredicate<int>
        {
            ...
            ref var list = ref bodies3D.ActiveSet.Constraints[bodyIndex];
        ...
        /// <summary>
        /// Traverses the active constraint graph collecting bodies3D that match a predicate. If any body visited during the traversal fails to match the predicate, the traversal terminates.
        /// </summary>
        ...
        /// <param name="bodyIndices">List to fill with body indices traversed during island collection. Bodies3D failing the predicate will not be included.</param>
        ...
        /// The bodyIndices and constraintHandles lists will contain all traversed predicate-passing bodies3D and constraints.</returns>
        ...
        {
            Debug.Assert(startingActiveBodyIndex >= 0 && startingActiveBodyIndex < bodies3D.ActiveSet.Count);
            //We'll build the island by working depth-first. This means the bodies3D and constraints we accumulate will be stored in any inactive island by depth-first order,
            //which happens to be a pretty decent layout for cache purposes. In other words, when we wake these islands back up, bodies3D near each other in the graph will have 
            //a higher chance of being near each other in memory. Bodies3D directly connected may often end up adjacent to each other, meaning loading one body may give you the other for 'free'
            ...
            var initialBodyCapacity = Math.Min(InitialIslandBodyCapacity, bodies3D.ActiveSet.Count);
            //Note that we track all considered bodies3D AND constraints. 
            //While we only need to track one of them for the purposes of traversal, tracking both allows low-overhead collection of unique bodies3D and constraints.
            ...
            var consideredBodies = new IndexSet(pool, bodies3D.ActiveSet.Count);
        ...
        void FindIslands<TPredicate>(int workerIndex, BufferPool threadPool, ref TPredicate predicate) where TPredicate : IPredicate<int>
        {
            ...
            var bodyIndices = new QuickList<int>(Math.Min(InitialIslandBodyCapacity, bodies3D.ActiveSet.Count), threadPool);
            ...
            traversalTest.PreviouslyTraversedBodies = new IndexSet(threadPool, bodies3D.ActiveSet.Count);
        ...
        void FindIslands(int workerIndex, BufferPool threadPool)
        {
            ...
            {
                var predicate = new SleepPredicate { bodies3D = bodies3D };
                FindIslands(workerIndex, threadPool, ref predicate);
            }
        }
        ...
        unsafe void Gather(int workerIndex)
        {
            while (true)
            {
                ...
                if (job.IsBodyJob)
                {
                    //Load a range of bodies3D from the active set and store them into the target inactive body set.
                    ref var sourceSet = ref bodies3D.ActiveSet;
                    ...
                    for (int targetIndex = job.StartIndex; targetIndex < job.EndIndex; ++targetIndex)
                    {
                        ...
                        ref var targetSet = ref bodies3D.Sets[job.TargetSetIndex];
                ...
                else
                {
                    ...
                    solver.TypeProcessors[targetTypeBatch.TypeId].GatherActiveConstraints(bodies3D, solver, ref job.SourceIndices, job.StartIndex, job.EndIndex, ref targetTypeBatch);
                ...
                case RemovalJobType.RemoveBodiesFromActiveSet:
                    {
                        for (int setReferenceIndex = 0; setReferenceIndex < newInactiveSets.Count; ++setReferenceIndex)
                        {
                            ...
                            ref var inactiveBodySet = ref bodies3D.Sets[setIndex];
                            ref var inactiveConstraintSet = ref solver.Sets[setIndex];
                            for (int bodyIndex = 0; bodyIndex < inactiveBodySet.Count; ++bodyIndex)
                            {
                                ref var location = ref bodies3D.HandleToLocation[inactiveBodySet.IndexToHandle[bodyIndex]];
                                ...
                                bodies3D.RemoveFromActiveSet(location.Index);
                ...
                case RemovalJobType.AddCollidablesToStaticTree:
                    {
                        for (int setReferenceIndex = 0; setReferenceIndex < newInactiveSets.Count; ++setReferenceIndex)
                        {
                            ...
                            ref var set = ref bodies3D.Sets[setReference.Index];
                ...
                case RemovalJobType.NotifyNarrowPhasePairCache:
                    {
                        ...
                        //we can approximate it based on bodies3D.
                        int largestBodyCount = 0;
                        for (int i = 0; i < newInactiveSets.Count; ++i)
                        {
                            var setCount = bodies3D.Sets[newInactiveSets[i].Index].Count;
        ...
        unsafe void PrintIsland(ref IslandScaffold island)
        {
            ...
            for (int i = 0; i < island.BodyIndices.Count; ++i)
                Console.Write($"{bodies3D.ActiveSet.IndexToHandle[island.BodyIndices[i]]}, ");
            Console.WriteLine();
            ...
            for (int batchIndex = 0; batchIndex < island.Protobatches.Count; ++batchIndex)
            {
                ...
                for (int typeBatchIndex = 0; typeBatchIndex < batch.TypeBatches.Count; ++typeBatchIndex)
                {
                    ...
                    for (int indexInTypeBatch = 0; indexInTypeBatch < typeBatch.Handles.Count; ++indexInTypeBatch)
                    {
                        ...
                        {
                            constraintReferencedBodyHandles.AddRef(ref bodies3D.ActiveSet.IndexToHandle[references[i]], pool);
                        }
        ...
        unsafe void Sleep(ref QuickList<int> traversalStartBodyIndices, IThreadDispatcher threadDispatcher, bool deterministic, int targetSleptBodyCount, int targetTraversedBodyCount, bool forceSleep)
        {
            ...
            //2) Gathering the data backing the bodies3D and constraints of a simulation island and placing it into an inactive storage representation (a BodySet and ConstraintSet).
            //3) Removing bodies3D, some solver bookkeeping related to removed constraints, and broad phase work.
            ...
            if (bodies3D.ActiveSet.Count == 0 || traversalStartBodyIndices.Count == 0)
                return;
            ...
            //2) GATHERING
            //Traversal is now done. We should have a set of results for each worker in the workerTraversalResults. It's time to gather all the data for the slept bodies3D and constraints.
            ...
            //Each worker output a set of their traversed bodies3D. Using it, we can efficiently check to see if a given island is a duplicate by checking all previous workers.
            ...
            for (int workerIndex = 0; workerIndex < workerTraversalThreadCount; ++workerIndex)
            {
                ...
                for (int j = 0; j < workerIslands.Count; ++j)
                {
                    ...
                    {
                        Debug.Assert(island.BodyIndices.Count > 0, "Any reported island should have a positive number of bodies3D in it. Otherwise, there's nothing to sleep!");
                    ...
                    if (!skip)
                    {
                        ...
                        bodies3D.Sets[setIndex] = new BodySet(island.BodyIndices.Count, pool);
                        bodies3D.Sets[setIndex].Count = island.BodyIndices.Count;
                        ...
                        //A job also only covers either bodies3D or constraints, not both at once.
            ...
            //Stage 4 only removes constraints from type batches. Still need to update active set constraint batches, bodies3D, and so on.
            ...
            //-Body removal results in moved bodies3D, and any constraints associated with moved bodies3D must have their body indices updated to point at the new location.
        ...
        /// <summary>
        /// Forcefully sleeps a list of bodies3D and all bodies3D that can be reached by traversing the constraint graph from those bodies3D.
        /// </summary>
        ...
        /// <summary>
        /// Forces a body and all bodies3D that can be found by traversing the constraint graph from that body to go to sleep.
        /// </summary>
        ...
        {
            if (bodies3D.ActiveSet.Count == 0)
                return;

            int candidateCount = (int)Math.Max(1, bodies3D.ActiveSet.Count * TestedFractionPerFrame);
            ...
            int spacing = bodies3D.ActiveSet.Count / candidateCount;
            ...
            if (scheduleOffset > bodies3D.ActiveSet.Count)
            {
                scheduleOffset = 0;
            }
            ...
            {
                if (index >= bodies3D.ActiveSet.Count)
                {
                    index -= bodies3D.ActiveSet.Count;
                }
            ...
            if (deterministic)
            {
                ...
                pool.Take<int>(bodies3D.ActiveSet.Count, out var sortedIndices);
                for (int i = 0; i < bodies3D.ActiveSet.Count; ++i)
                {
                    sortedIndices[i] = i;
                }
                //Handles are guaranteed to be unique; no need for three way partitioning.
                HandleComparer comparer;
                comparer.Handles = bodies3D.ActiveSet.IndexToHandle;
                ...
                QuickSort.Sort(ref sortedIndices[0], 0, bodies3D.ActiveSet.Count - 1, ref comparer);
                ...
                for (int i = 0; i < traversalStartBodyIndices.Count; ++i)
                {
                    ...
                    Debug.Assert(traversalStartBodyIndices[i] >= 0 && traversalStartBodyIndices[i] < bodies3D.ActiveSet.Count);
                }
                pool.Return(ref sortedIndices);
            }
            Sleep(ref traversalStartBodyIndices, threadDispatcher, deterministic, (int)Math.Ceiling(bodies3D.ActiveSet.Count * TargetSleptFraction), (int)Math.Ceiling(bodies3D.ActiveSet.Count * TargetTraversedFraction), false);

            traversalStartBodyIndices.Dispose(pool);
        }

        /// <summary>
        /// Ensures that the Bodies3D, Solver, and NarrowPhase can hold at least the given number of sets (BodySets for the Bodies3D collection, ConstraintSets for the Solver, PairSubcaches for the NarrowPhase.PairCache).
        /// </summary>
        /// <param name="setsCapacity">Number of sets to guarantee space for.</param>
        public void EnsureSetsCapacity(int setsCapacity)
        {
            var potentiallyAllocatedCount = Math.Min(setIdPool.HighestPossiblyClaimedId + 1, Math.Min(bodies3D.Sets.Length, Math.Min(solver.Sets.Length, pairCache.SleepingSets.Length)));
            if (setsCapacity > bodies3D.Sets.Length)
            {
                bodies3D.ResizeSetsCapacity(setsCapacity, potentiallyAllocatedCount);
            }
        ...
        /// <summary>
        /// Ensures that the Bodies3D and Solver can hold the given number of sets. 
