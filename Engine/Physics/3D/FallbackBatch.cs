...
namespace Engine.Physics._3D
{
    ...
    /// <summary>
    /// Contains constraints that could not belong to any lower constraint batch due to their involved bodies3D. All of the contained constraints will be solved using a fallback solver that
    ...
    public struct FallbackBatch
    {
        ...
        /// <summary>
        /// Gets the number of bodies3D in the fallback batch.
        /// </summary>
        ...
        {
            int GetBodyReference(Bodies3D bodies3D, int handle);
        }
        ...
        struct ActiveSetGetter : IBodyReferenceGetter
        {
            ...
            public int GetBodyReference(Bodies3D bodies3D, int bodyHandle)
            {
                ...
                Debug.Assert(bodyLocation.SetIndex == 0, "When creating a fallback batch for the active set, all bodies3D associated with it must be active.");
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal unsafe void AllocateForActive(int handle, ref int constraintBodyHandles, int bodyCount, Bodies3D bodies3D,
           int typeId, BufferPool pool, int minimumBodyCapacity = 8, int minimumReferenceCapacity = 8)
        {
            Allocate(handle, ref constraintBodyHandles, bodyCount, bodies3D, typeId, pool, new ActiveSetGetter(), minimumBodyCapacity, minimumReferenceCapacity);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void AllocateForInactive(int handle, ref int constraintBodyHandles, int bodyCount, Bodies3D bodies3D,
          int typeId, BufferPool pool, int minimumBodyCapacity = 8, int minimumReferenceCapacity = 8)
        {
            Allocate(handle, ref constraintBodyHandles, bodyCount, bodies3D, typeId, pool, new InactiveSetGetter(), minimumBodyCapacity, minimumReferenceCapacity);
        }
        ...
        public void GetJacobiScaleForBodies(ref TwoBodyReferences references, int count, out Vector<float> jacobiScaleA, out Vector<float> jacobiScaleB)
        {
            ...
            Vector<int> countsA = default, countsB = default;
            ...
            out Vector<float> jacobiScaleA, out Vector<float> jacobiScaleB, out Vector<float> jacobiScaleC)
        {
            ...
            Vector<int> countsA = default, countsB = default, countsC = default;
            ref var countsAStart = ref Unsafe.As<Vector<int>, int>(ref countsA);
            ref var countsBStart = ref Unsafe.As<Vector<int>, int>(ref countsB);
            ...
            out Vector<float> jacobiScaleA, out Vector<float> jacobiScaleB, out Vector<float> jacobiScaleC, out Vector<float> jacobiScaleD)
        {
            ...
            Vector<int> countsA = default, countsB = default, countsC = default, countsD = default;
            ref var countsAStart = ref Unsafe.As<Vector<int>, int>(ref countsA);
            ref var countsBStart = ref Unsafe.As<Vector<int>, int>(ref countsB);
            ref var countsCStart = ref Unsafe.As<Vector<int>, int>(ref countsC);
                ...
                for (int typeBatchIndex = 0; typeBatchIndex < batch.TypeBatches.Count; ++typeBatchIndex)
                {
                    ...
                    for (int constraintIndex = 0; constraintIndex < typeBatch.ConstraintCount; ++constraintIndex)
                    {
                        ...
                        for (int i = 0; i < bodiesPerConstraint; ++i)
                        {
                            ...
                            Debug.Assert(constraintIndexInBodySet >= 0, "Any constraint in the fallback batch should be in all connected bodies3D' constraint handle listings.");
        ...
        public void ScatterVelocities(Bodies3D bodies3D, Solver solver, ref Buffer<FallbackTypeBatchResults> velocities, int start, int exclusiveEnd)
        {
            ...
            for (int i = start; i < exclusiveEnd; ++i)
            {
                ...
                //to having a linear/angular 'weld' constraint between N separate bodies3D that happen to all be in the same spot, except each of them has 1/N as much mass as the original.
                ...
                bodies3D.ActiveSet.Velocities[bodyIndex] = bodyVelocity;
            }
        }
        ...
        internal void EnsureCapacity(int bodyCapacity, BufferPool pool)
        {
            ...
            {
                //This is conservative since there's no guarantee that we'll actually need to resize at all if these bodies3D are already present, but that's fine. 
