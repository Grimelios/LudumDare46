...
namespace Engine.Physics._3D.CollisionDetection
{
    ...
    struct CollisionBatch
    {
        ...
        public UntypedBlob PhysicsShapes3D;
    }
    ...
    public struct CollisionBatcher<TCallbacks> where TCallbacks : struct, ICollisionCallbacks
    {
        ...
        //Consider two compound bodies3D colliding. The pair will decompose into a set of potentially many convex subpairs.
            ...
            in PairContinuation continuation)
        {
                ...
                case CollisionTaskPairType.SpherePair:
                    {
                        ...
                        pair.A = Unsafe.AsRef<PhysicsSphere>(shapeA);
                        pair.B = Unsafe.AsRef<PhysicsSphere>(shapeB);
                        pair.OffsetB = offsetB;
                        pair.SpeculativeMargin = speculativeMargin;
                        pair.Continuation = continuation;
                    }
            ...
            if (batch.Pairs.Count == reference.BatchSize)
            {
                ...
                batch.PhysicsShapes3D.ByteCount = 0;
            }
        }
            ...
            in PairContinuation continuation)
        {
            ...
            PhysicsShapes3D[shapeIndexA.Type].GetShapeData(shapeIndexA.Index, out var shapeA, out var shapeSizeA);
            PhysicsShapes3D[shapeIndexB.Type].GetShapeData(shapeIndexB.Index, out var shapeB, out var shapeSizeB);
        ...
        unsafe void CacheShapes(ref CollisionTaskReference reference, void* shapeA, void* shapeB, int shapeSizeA, int shapeSizeB, out void* cachedShapeA, out void* cachedShapeB)
        {
            ...
            ref var batch = ref batches[reference.TaskIndex];
            if (!batch.PhysicsShapes3D.Buffer.Allocated)
            {
                var size = reference.BatchSize * (shapeSizeA + shapeSizeB);
                Pool.TakeAtLeast(size, out batch.PhysicsShapes3D.Buffer);
                Debug.Assert(batch.PhysicsShapes3D.ByteCount == 0);
            }
            cachedShapeA = batch.PhysicsShapes3D.Allocate(shapeSizeA);
            cachedShapeB = batch.PhysicsShapes3D.Allocate(shapeSizeB);
        ...
        public unsafe void CacheShapeB(int shapeTypeA, int shapeTypeB, void* shapeDataB, int shapeSizeB, out void* cachedShapeDataB)
        {
            ...
            if (!batch.PhysicsShapes3D.Buffer.Allocated)
            {
                var size = reference.BatchSize * (PhysicsShapes3D[shapeTypeA].ShapeDataSize + shapeSizeB);
                Pool.TakeAtLeast(size, out batch.PhysicsShapes3D.Buffer);
                Debug.Assert(batch.PhysicsShapes3D.ByteCount == 0);
            }
            cachedShapeDataB = batch.PhysicsShapes3D.Allocate(shapeSizeB);
        ...
        {
            //Note that the physicsShapes3D are passed by copy to avoid a GC hole. This isn't optimal, but it does allow a single code path, and the underlying function is the one
        ...
        public void Flush()
        {
            ...
            for (int i = minimumBatchIndex; i <= maximumBatchIndex; ++i)
            {
                ...
                if (batch.PhysicsShapes3D.Buffer.Allocated)
                {
                    Pool.Return(ref batch.PhysicsShapes3D.Buffer);
                }
            }
