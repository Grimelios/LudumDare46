...
namespace Engine.Physics._3D.CollisionDetection
{
    internal class FreshnessChecker
    {
        ...
        unsafe void PrintRemovalInformation(int constraintHandle)
        {
            ...
            {
                var bodyHandle = constraintRemover.bodies3D.ActiveSet.IndexToHandle[references[i]];
                Console.Write($"{bodyHandle} ({constraintRemover.bodies3D.ActiveSet.Constraints[references[i]].Count}), ");
