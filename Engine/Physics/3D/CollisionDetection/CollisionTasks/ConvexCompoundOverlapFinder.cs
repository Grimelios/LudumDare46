...
namespace Engine.Physics._3D.CollisionDetection.CollisionTasks
{
    ...
    {
        unsafe void FindLocalOverlaps<TOverlaps, TSubpairOverlaps>(ref Buffer<OverlapQueryForPair> pairs, BufferPool pool, PhysicsShapes3D shapes, ref TOverlaps overlaps)
            where TOverlaps : struct, ICollisionTaskOverlaps<TSubpairOverlaps>
            where TSubpairOverlaps : struct, ICollisionTaskSubpairOverlaps;

        unsafe void FindLocalOverlaps<TOverlaps>(in Vector3 min, in Vector3 max, in Vector3 sweep, float maximumT, BufferPool pool, PhysicsShapes3D shapes, void* overlaps)
            where TOverlaps : ICollisionTaskSubpairOverlaps;
    }

    public interface IConvexCompoundOverlapFinder
    {
        void FindLocalOverlaps(ref Buffer<BoundsTestedPair> pairs, int pairCount, BufferPool pool, PhysicsShapes3D shapes, float dt, out ConvexCompoundTaskOverlaps overlaps);
    }
