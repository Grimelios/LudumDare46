...
namespace Engine.Physics._3D.CollisionDetection.CollisionTasks
{
    public struct CapsuleCylinderTester : IPairTester<CapsuleWide, CylinderWide, Convex2ContactManifoldWide>
    {
        ...
        public void Test(ref CapsuleWide a, ref CylinderWide b, ref Vector<float> speculativeMargin, ref Vector3Wide offsetB, ref QuaternionWide orientationA, ref QuaternionWide orientationB, int pairCount, out Convex2ContactManifoldWide manifold)
        {
            ...
            //PhysicsCapsule3D endpoint vs cylinder cap plane :
            //PhysicsCapsule3D endpoint vs cylinder cap edge  : Only applies during shallow collision; if the capsule line segment intersects the cylinder, the fastest way out for an endpoint is straight up or sideways, not diagonally.
            //PhysicsCapsule3D endpoint vs cylinder side      :
            //PhysicsCapsule3D edge     vs cylinder cap plane : redundant with endpoint-plane; the capsule segment cannot be closer than either of its endpoints to any plane.
            //PhysicsCapsule3D edge     vs cylinder cap edge  : this one is nasty!
            //PhysicsCapsule3D edge     vs cylinder side      : provided the other cases are covered, this is simply a closest point between lines test (internal line of capsule versus internal line of cylinder).

            //PhysicsCapsule3D edge versus cylinder cap edge is nasty. It involves finding the roots of a fourth degree polynomial; we're going to avoid doing that.
            ...
            //PhysicsCapsule3D segment  vs cylinder           : numerically solved
            //PhysicsCapsule3D endpoint vs cylinder cap plane : only needed if any lane failed the shallow distance test
            //PhysicsCapsule3D segment  vs cylinder side      : combined endpoint and edge tests; only needed if any lane failed the shallow distance test
            ...
            //PhysicsCapsule3D segment vs cylinder side
            //PhysicsCapsule3D segment vs cylinder cap
