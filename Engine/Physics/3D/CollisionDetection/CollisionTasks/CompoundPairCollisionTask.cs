...
namespace Engine.Physics._3D.CollisionDetection.CollisionTasks
{
    ...
    {
        void FindLocalOverlaps(ref Buffer<BoundsTestedPair> pairs, int pairCount, BufferPool pool, PhysicsShapes3D shapes, float dt, out CompoundPairOverlaps overlaps);
    }
        ...
        where TContinuation : struct, ICollisionTestContinuation
    {
        ...
        public unsafe override void ExecuteBatch<TCallbacks>(ref UntypedList batch, ref CollisionBatcher<TCallbacks> batcher)
        {
            ...
            overlapFinder.FindLocalOverlaps(ref pairs, batch.Count, batcher.Pool, batcher.PhysicsShapes3D, batcher.Dt, out var overlaps);
                ...
                if (totalOverlapCountForPair > 0)
                {
                    ...
                    for (int j = 0; j < pairOverlaps.Length; ++j)
                    {
                        ...
                        for (int k = 0; k < childOverlaps.Count; ++k)
                        {
                            ...
                            //Note that we have to take into account whether we flipped the physicsShapes3D to match the expected memory layout.
