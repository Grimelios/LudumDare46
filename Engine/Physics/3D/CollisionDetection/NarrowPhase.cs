...
namespace Engine.Physics._3D.CollisionDetection
{
    ...
    public partial class NarrowPhase<TCallbacks> : NarrowPhase where TCallbacks : struct, INarrowPhaseCallbacks
    {
        ...
        public struct OverlapWorker
        {
            ...
            {
                Batcher = new CollisionBatcher<CollisionCallbacks>(pool, narrowPhase.PhysicsShapes3D, narrowPhase.CollisionTaskRegistry, narrowPhase.timestepDuration,
            ...
            : base()
        {
            ...
            PhysicsShapes3D = simulation.PhysicsShapes3D;
            bodies3D = simulation.Bodies3D;
        ...
        public unsafe void HandleOverlap(int workerIndex, CollidableReference a, CollidableReference b)
        {
            ...
            //Between two bodies3D, simply put the lower handle in slot A always.
            ...
            {
                //Both references are bodies3D.
                ref var bodyLocationA = ref bodies3D.HandleToLocation[a.Handle];
                ref var bodyLocationB = ref bodies3D.HandleToLocation[b.Handle];
                Debug.Assert(bodyLocationA.SetIndex == 0 || bodyLocationB.SetIndex == 0, "One of the two bodies3D must be active. Otherwise, something is busted!");
                ref var setA = ref bodies3D.Sets[bodyLocationA.SetIndex];
                ref var setB = ref bodies3D.Sets[bodyLocationB.SetIndex];
            ...
            else
            {
                ...
                //Further, we know that the body must be an *active* body, because inactive bodies3D and statics exist within the same static/inactive broad phase tree and are not tested
                //against each other.
                Debug.Assert(aMobility != CollidableMobility.Static && bMobility == CollidableMobility.Static);
                ref var bodyLocation = ref bodies3D.HandleToLocation[a.Handle];
                ...
                ref var bodySet = ref bodies3D.ActiveSet;
                AddBatchEntries(workerIndex, ref overlapWorker, ref pair,
            ...
            ref RigidPose poseA, ref RigidPose poseB, ref BodyVelocity velocityA, ref BodyVelocity velocityB)
        {
            ...
            if (aCollidable.Continuity.Mode == ContinuousDetectionMode.Continuous || bCollidable.Continuity.Mode == ContinuousDetectionMode.Continuous)
            {
                ...
                if (sweepTask != null)
                {
                    //Not every continuous pair requires an actual sweep test. If the maximum approaching displacement for any point on the involved physicsShapes3D isn't any larger
                    ...
                    var aInStaticTree = pair.A.Mobility == CollidableMobility.Static || simulation.Bodies3D.HandleToLocation[pair.A.Handle].SetIndex > 0;
                    var bInStaticTree = pair.B.Mobility == CollidableMobility.Static || simulation.Bodies3D.HandleToLocation[pair.B.Handle].SetIndex > 0;
                    ...
                    {
                        simulation.PhysicsShapes3D[aCollidable.Shape.Type].GetShapeData(aCollidable.Shape.Index, out var shapeDataA, out var shapeSizeA);
                        simulation.PhysicsShapes3D[bCollidable.Shape.Type].GetShapeData(bCollidable.Shape.Index, out var shapeDataB, out var shapeSizeB);
                        ...
                        if (sweepTask.Sweep(
                            ...
                            ref filter, simulation.PhysicsShapes3D, SweepTaskRegistry, overlapWorker.Batcher.Pool, out _, out var t1, out _, out _))
                        {
