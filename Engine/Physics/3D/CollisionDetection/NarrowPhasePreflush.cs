...
namespace Engine.Physics._3D.CollisionDetection
{
    ...
    public partial class NarrowPhase<TCallbacks>
    {
        ...
        protected override void OnPreflush(IThreadDispatcher threadDispatcher, bool deterministic)
        {
            ...
            var uniqueAwakeningsSet = new IndexSet(Pool, simulation.Bodies3D.Sets.Length);
            for (int i = 0; i < threadCount; ++i)
            {
            ...
            else
            {
                ...
                //This this was not done, pending adds might end up in the same batches as newly awake constraints that share bodies3D.
                for (int i = 0; i < awakenerPhaseTwoJobCount; ++i)
