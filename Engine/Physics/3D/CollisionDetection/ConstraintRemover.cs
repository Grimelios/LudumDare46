...
namespace Engine.Physics._3D.CollisionDetection
{
    ...
    public class ConstraintRemover
    {
        ...
        struct WorkerCache
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public unsafe void EnqueueForRemoval(int constraintHandle, Solver solver, Bodies3D bodies3D)
            {
                ...
                for (int i = 0; i < bodiesPerConstraint; ++i)
                {
                    ...
                    target.BodyHandle = bodies3D.ActiveSet.IndexToHandle[target.BodyIndex];
                }
            }
        ...
        public ConstraintRemover(BufferPool pool, Bodies3D bodies3D, Solver solver, int minimumTypeCapacity = 4, int minimumRemovalCapacity = 128, float previousCapacityMultiplier = 1.25f)
        {
            this.pool = pool;
        ...
        public void RemoveConstraintsFromBodyLists()
        {
            ...
            for (int i = 0; i < batches.BatchCount; ++i)
            {
                ...
                for (int j = 0; j < removals.Count; ++j)
                {
                    ...
                    bodies3D.RemoveConstraintReference(target.BodyIndex, target.ConstraintHandle);
                }
            }
        }
        ...
        {
            workerCaches[workerIndex].EnqueueForRemoval(constraintHandle, solver, bodies3D);
        }
    }
}
