...
{
    /// <summary>
    /// Associated with a pair of two collidables that each are controlled by bodies3D.
    /// </summary>
    ...
    public partial class NarrowPhase<TCallbacks> where TCallbacks : struct, INarrowPhaseCallbacks
    {
            ...
            where TContactImpulses : unmanaged
        {
            ...
            else
            {
                ...
                //Note that this is only possible when both colliders are bodies3D. If only one collider is a body, then it must be active otherwise this pair would never have been tested.
                if (typeof(TBodyHandles) == typeof(TwoBodyHandles))
                {
                    ...
                    ref var locationA = ref bodies3D.HandleToLocation[twoBodyHandles.A];
                    ref var locationB = ref bodies3D.HandleToLocation[twoBodyHandles.B];
