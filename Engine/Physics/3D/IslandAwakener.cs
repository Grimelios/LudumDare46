...
{
    /// <summary>
    /// Provides functionality for efficiently waking up sleeping bodies3D.
    /// </summary>
    public class IslandAwakener
    {
        ...
        /// <summary>
        /// Wakes up a body if it is sleeping. All bodies3D that can be found by traversing the constraint graph from the body will also be awakened.
        ...
        public void AwakenBody(int bodyHandle)
        {
            ...
            AwakenSet(bodies3D.HandleToLocation[bodyHandle].SetIndex);
        }

        /// <summary>
        /// Wakes up any sleeping bodies3D associated with a constraint. All bodies3D that can be found by traversing the constraint graph from the constraint referenced bodies3D will also be awakened.
        /// If all bodies3D associated with the constraint are already awake, this does nothing.
        /// </summary>
        ...
        /// <summary>
        /// Wakes up all bodies3D and constraints within a set. Doesn't do anything if the set is awake (index zero).
        /// </summary>
                        ...
                        for (int i = 0; i < uniqueSetIndices.Count; ++i)
                        {
                            ...
                            if (source.bodyConstraintReferences.Count > 0)
                            {
                                for (int j = 0; j < source.bodyConstraintReferences.Count; ++j)
                                {
                                    ...
                                    ref var bodyLocation = ref bodies3D.HandleToLocation[source.bodyConstraintReferences.Keys[j]];
                                    Debug.Assert(bodyLocation.SetIndex == 0, "Any batch moved into the active set should be dealing with bodies3D which have already been moved into the active set.");
                ...
                case PhaseOneJobType.CopyBodyRegion:
                    {
                        ...
                        ref var sourceSet = ref bodies3D.Sets[job.SourceSet];
                        ref var targetSet = ref bodies3D.ActiveSet;
                        ...
                        for (int i = 0; i < job.Count; ++i)
                        {
                            ...
                            ref var targetWorldInertia = ref bodies3D.Inertias[targetIndex];
        ...
        internal unsafe void ExecutePhaseTwoJob(int index)
        {
            ...
            switch (job.Type)
            {
                case PhaseTwoJobType.BroadPhase:
                    {
                        ...
                        ref var activeSet = ref bodies3D.ActiveSet;
                        for (int i = 0; i < uniqueSetIndices.Count; ++i)
                        {
                            ref var sleepingBodySet = ref bodies3D.Sets[uniqueSetIndices[i]];
                            for (int j = 0; j < sleepingBodySet.Count; ++j)
                            {
                                ...
                                ref var bodyLocation = ref bodies3D.HandleToLocation[sleepingBodySet.IndexToHandle[j]];
                                ...
                                if (broadPhaseIndex >= 0)
                                {
                                    ...
                                    if (broadPhase.RemoveStaticAt(staticBroadPhaseIndexToRemove, out var movedLeaf))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            bodies3D.UpdateCollidableBroadPhaseIndex(movedLeaf.Handle, staticBroadPhaseIndexToRemove);
                                        }
                                    }
                                }
                            }
                ...
                case PhaseTwoJobType.CopyConstraintRegion:
                    {
                        //Constraints are a little more complicated than bodies3D for a few reasons:
                        ...
                        //3) The translation from body handle to body index requires that the bodies3D already have an active set identity, which is why the constraints wait until the second phase.
                        ...
                        solver.TypeProcessors[job.TypeId].CopySleepingToActive(
                            ...
                            job.SourceStart, job.TargetStart, job.Count, bodies3D, solver);
                    }
                    break;
            }
        ...
        {
            Debug.Assert(setIndex >= 1 && setIndex < bodies3D.Sets.Length && setIndex < solver.Sets.Length && setIndex < pairCache.SleepingSets.Length);
            ...
            Debug.Assert(bodies3D.Sets[setIndex].Allocated);
        }
        [Conditional("DEBUG")]
        void ValidateUniqueSets(ref QuickList<int> setIndices)
        {
            var set = new IndexSet(pool, bodies3D.Sets.Length);
        ...
        unsafe internal (int phaseOneJobCount, int phaseTwoJobCount) PrepareJobs(ref QuickList<int> setIndices, bool resetActivityStates, int threadCount)
        {
            ...
            //1) Ensure that the active set in the bodies3D, solver, and pair cache can hold the newly awakened islands without resizing or accessing a buffer pool.
            //2) Allocate space in the active set for the bodies3D and constraints.
            ...
            {
                var setIndex = setIndices[i];
                newBodyCount += bodies3D.Sets[setIndex].Count;
            ...
            //We now know how many new bodies3D, constraint batch entries, and pair cache entries are going to be added.
            //Ensure capacities on all systems:
            //bodies3D,
            bodies3D.EnsureCapacity(bodies3D.ActiveSet.Count + newBodyCount);
            ...
            for (int batchIndex = solver.ActiveSet.Batches.Count; batchIndex < highestNewBatchCount; ++batchIndex)
            {
                ...
                {
                    solver.batchReferencedHandles.AllocateUnsafely() = new IndexSet(pool, bodies3D.HandlePool.HighestPossiblyClaimedId + 1);
                }
            }
            for (int batchIndex = 0; batchIndex < highestNewBatchCount; ++batchIndex)
            {
                ...
                if (batchIndex < solver.FallbackBatchThreshold)
                {
                    solver.batchReferencedHandles[batchIndex].EnsureCapacity(bodies3D.HandlePool.HighestPossiblyClaimedId + 1, pool);
                }
            ...
            //Finally, create actual jobs. Note that this involves actually allocating space in the bodies3D set and in type batches for the workers to fill in.
            ...
            ref var activeBodySet = ref bodies3D.ActiveSet;
            ...
            for (int i = 0; i < uniqueSetIndices.Count; ++i)
            {
                var sourceSetIndex = uniqueSetIndices[i];
                {
                    ...
                    ref var sourceSet = ref bodies3D.Sets[sourceSetIndex];
                    ...
                    for (int jobIndex = 0; jobIndex < setJobCount; ++jobIndex)
                    {
                        ...
                        for (int j = 0; j < job.Count; ++j)
                        {
                            ...
                            ref var bodyLocation = ref bodies3D.HandleToLocation[sourceSet.IndexToHandle[sourceIndex]];
        ...
        internal void DisposeForCompletedAwakenings(ref QuickList<int> setIndices)
        {
            for (int i = 0; i < setIndices.Count; ++i)
            {
                ...
                //Note that neither the constraint set nor the pair cache set necessarily exist. It is possible for bodies3D to go to sleep by themselves.
                ref var constraintSet = ref solver.Sets[setIndex];
