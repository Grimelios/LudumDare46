    ...
    public partial class Solver
    {
        ...
        struct PrestepStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                if (block.BatchIndex < solver.FallbackBatchThreshold)
                    typeProcessor.Prestep(ref typeBatch, solver.bodies3D, Dt, InverseDt, block.StartBundle, block.End);
                else
                    typeProcessor.JacobiPrestep(ref typeBatch, solver.bodies3D, ref solver.ActiveSet.Fallback, Dt, InverseDt, block.StartBundle, block.End);
            }
        }
        struct WarmStartStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                ref var typeBatch = ref solver.ActiveSet.Batches[block.BatchIndex].TypeBatches[block.TypeBatchIndex];
                var typeProcessor = solver.TypeProcessors[typeBatch.TypeId];
                typeProcessor.WarmStart(ref typeBatch, ref solver.bodies3D.ActiveSet.Velocities, block.StartBundle, block.End);
        ...
        struct SolveStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                ref var typeBatch = ref solver.ActiveSet.Batches[block.BatchIndex].TypeBatches[block.TypeBatchIndex];
                var typeProcessor = solver.TypeProcessors[typeBatch.TypeId];
                typeProcessor.SolveIteration(ref typeBatch, ref solver.bodies3D.ActiveSet.Velocities, block.StartBundle, block.End);
            }
        }
        ...
        struct WarmStartFallbackStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                ref var typeBatch = ref solver.ActiveSet.Batches[block.BatchIndex].TypeBatches[block.TypeBatchIndex];
                var typeProcessor = solver.TypeProcessors[typeBatch.TypeId];
                typeProcessor.JacobiWarmStart(ref typeBatch, ref solver.bodies3D.ActiveSet.Velocities, ref solver.context.FallbackResults[block.TypeBatchIndex], block.StartBundle, block.End);
        ...
        struct SolveFallbackStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                ref var typeBatch = ref solver.ActiveSet.Batches[block.BatchIndex].TypeBatches[block.TypeBatchIndex];
                var typeProcessor = solver.TypeProcessors[typeBatch.TypeId];
                typeProcessor.JacobiSolveIteration(ref typeBatch, ref solver.bodies3D.ActiveSet.Velocities, ref solver.context.FallbackResults[block.TypeBatchIndex], block.StartBundle, block.End);
            }
        }
        ...
        struct FallbackScatterStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                solver.ActiveSet.Fallback.ScatterVelocities(solver.bodies3D, solver, ref solver.context.FallbackResults, block.Start, block.End);
            }
        }
            ...
            if (threadDispatcher == null)
            {
                ...
                for (int i = 0; i < synchronizedBatchCount; ++i)
                {
                    ...
                    for (int j = 0; j < batch.TypeBatches.Count; ++j)
                    {
                        ...
                        TypeProcessors[typeBatch.TypeId].Prestep(ref typeBatch, bodies3D, dt, inverseDt, 0, typeBatch.BundleCount);
                    }
                }
                if (fallbackExists)
                {
                    ...
                    {
                        ref var typeBatch = ref batch.TypeBatches[j];
                        TypeProcessors[typeBatch.TypeId].JacobiPrestep(ref typeBatch, bodies3D, ref activeSet.Fallback, dt, inverseDt, 0, typeBatch.BundleCount);
                    }
                }
                ...
                for (int i = 0; i < synchronizedBatchCount; ++i)
                {
                    ...
                    {
                        ref var typeBatch = ref batch.TypeBatches[j];
                        TypeProcessors[typeBatch.TypeId].WarmStart(ref typeBatch, ref bodies3D.ActiveSet.Velocities, 0, typeBatch.BundleCount);
                    }
                }
                ...
                if (fallbackExists)
                {
                    ...
                    {
                        ref var typeBatch = ref batch.TypeBatches[j];
                        TypeProcessors[typeBatch.TypeId].JacobiWarmStart(ref typeBatch, ref bodies3D.ActiveSet.Velocities, ref fallbackResults[j], 0, typeBatch.BundleCount);
                    }
                    activeSet.Fallback.ScatterVelocities(bodies3D, this, ref fallbackResults, 0, activeSet.Fallback.BodyCount);
                }
                for (int iterationIndex = 0; iterationIndex < iterationCount; ++iterationIndex)
                {
                    for (int i = 0; i < synchronizedBatchCount; ++i)
                    {
                        ...
                        {
                            ref var typeBatch = ref batch.TypeBatches[j];
                            TypeProcessors[typeBatch.TypeId].SolveIteration(ref typeBatch, ref bodies3D.ActiveSet.Velocities, 0, typeBatch.BundleCount);
                        }
                    }
                    if (fallbackExists)
                    {
                        ...
                        {
                            ref var typeBatch = ref batch.TypeBatches[j];
                            TypeProcessors[typeBatch.TypeId].JacobiSolveIteration(ref typeBatch, ref bodies3D.ActiveSet.Velocities, ref fallbackResults[j], 0, typeBatch.BundleCount);
                        }
                        activeSet.Fallback.ScatterVelocities(bodies3D, this, ref fallbackResults, 0, activeSet.Fallback.BodyCount);
                    }
                }
