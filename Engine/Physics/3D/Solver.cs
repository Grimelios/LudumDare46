...
namespace Engine.Physics._3D
{
    ...
    public partial class Solver
    {
        ...
        public Solver(Bodies3D bodies3D, BufferPool pool, int iterationCount, int fallbackBatchThreshold,
            int initialCapacity,
        ...
        {
            var maxBodySet = activeOnly ? 1 : bodies3D.Sets.Length;
            for (int i = 0; i < maxBodySet; ++i)
            {
                ref var set = ref bodies3D.Sets[i];
                if (set.Allocated)
                {
                    for (int j = 0; j < set.Count; ++j)
                    {
                        bodies3D.ValidateExistingHandle(set.IndexToHandle[j]);
                    }
                }
            }
            //Validate the bodies3D referenced in the active batchReferencedHandles collections. 
            ...
            for (int batchIndex = 0; batchIndex < Math.Min(ActiveSet.Batches.Count, FallbackBatchThreshold); ++batchIndex)
            {
                ...
                {
                    var handle = bodies3D.ActiveSet.IndexToHandle[i];
                    if (handles.Contains(handle))
                        ValidateBodyReference(i, 1, ref batch);
                    else
                        ValidateBodyReference(i, 0, ref batch);
                }
                //No inactive bodies3D should be present in the active set solver batch referenced handles.
                for (int inactiveBodySetIndex = 1; inactiveBodySetIndex < bodies3D.Sets.Length; ++inactiveBodySetIndex)
                {
                    ref var bodySet = ref bodies3D.Sets[inactiveBodySetIndex];
                    if (bodies3D.Sets[inactiveBodySetIndex].Allocated)
                    {
                        for (int i = 0; i < bodySet.Count; ++i)
                        {
                            Debug.Assert(!handles.Contains(bodySet.IndexToHandle[i]), "Bodies3D in an inactive set should not show up in the active solver set's referenced body handles.");
                        }
                    }
                }
            }
            ...
            for (int setIndex = 0; setIndex < maxConstraintSet; ++setIndex)
            {
                ...
                if (set.Allocated)
                {
                    Debug.Assert(bodies3D.Sets.Length > setIndex);
                    ...
                    ref var bodySet = ref bodies3D.Sets[setIndex];
                    ...
                    for (int batchIndex = 0; batchIndex < set.Batches.Count; ++batchIndex)
                    {
                        ...
                        for (int typebatchIndex = 0; typebatchIndex < batch.TypeBatches.Count; ++typebatchIndex)
                        {
                            ...
                            for (int indexInTypeBatch = 0; indexInTypeBatch < typeBatch.ConstraintCount; ++indexInTypeBatch)
                            {
                                ...
                                {
                                    //Active constraints refer to bodies3D with an index. Inactive constraints use a handle.
                                    int bodyIndex;
                                    if (setIndex == 0)
                                    {
                                    ...
                                    {
                                        ref var referencedBodyLocation = ref bodies3D.HandleToLocation[constraintBodyReferences[i]];
                    ...
                    for (int bodyIndex = 0; bodyIndex < bodySet.Count; ++bodyIndex)
                    {
                        ...
                        for (int constraintIndex = 0; constraintIndex < constraintList.Count; ++constraintIndex)
                        {
                            ...
                            //Active constraints refer to bodies3D by index; inactive constraints use handles.
        ...
        internal unsafe void AllocateInBatch(int targetBatchIndex, int constraintHandle, ref int bodyHandles, int bodyCount, int typeId, out ConstraintReference reference)
        {
            ...
            batch.Allocate(constraintHandle, ref bodyHandles, bodyCount, bodies3D, typeId, TypeProcessors[typeId], GetMinimumCapacityForType(typeId), pool, out reference);
            ...
            else
            {
                ...
                ActiveSet.Fallback.AllocateForActive(constraintHandle, ref bodyHandles, bodyCount, bodies3D, typeId, pool);
            }
        }
        ...
        internal unsafe bool TryAllocateInBatch(int typeId, int targetBatchIndex, ref int bodyHandles, int bodyCount, out int constraintHandle, out ConstraintReference reference)
        {
            ...
            if (targetBatchIndex == set.Batches.Count)
            {
                ...
                if (targetBatchIndex < FallbackBatchThreshold)
                {
                    ...
                    batchReferencedHandles.AllocateUnsafely() = new IndexSet(pool, bodies3D.ActiveSet.Count);
                }
            ...
            //Note that new constraints are always active. It is assumed at this point that all connected bodies3D have been forced active.
            for (int i = 0; i < bodyCount; ++i)
            {
                ref var bodyLocation = ref bodies3D.HandleToLocation[Unsafe.Add(ref bodyHandles, i)];
                Debug.Assert(bodyLocation.SetIndex == 0, "New constraints should only be created involving already-activated bodies3D.");
            }
        ...
        /// <summary>
        /// Applies a description to a constraint slot, waking up the connected bodies3D if necessary.
        /// </summary>
        /// <typeparam name="TDescription">Type of the description to apply.</typeparam>
        /// <param name="constraintHandle">Handle of the constraint being updated.</param>
        /// <param name="description">Description to apply to the slot.</param>
        ...
        /// <param name="bodyCount">Number of bodies3D used by the constraint.</param>
            ...
            where TDescription : IConstraintDescription<TDescription>
        {
            ...
            Debug.Assert(bodyCount == TypeProcessors[description.ConstraintTypeId].BodiesPerConstraint,
                "The number of bodies3D supplied to a constraint add must match the expected number of bodies3D involved in that constraint type. Did you use the wrong Solver.Add overload?");
            //Adding a constraint assumes that the involved bodies3D are active, so wake up anything that is sleeping.
            for (int i = 0; i < bodyCount; ++i)
            {
            ...
            for (int i = 0; i < bodyCount; ++i)
            {
                ...
                bodies3D.ValidateExistingHandle(bodyHandle);
                bodies3D.AddConstraint(bodies3D.HandleToLocation[bodyHandle].Index, constraintHandle, i);
            }
        ...
        {
            //Note that this function requires scanning the bodies3D in the constraint. This will tend to be fine since the vast majority of constraints have no more than 2 bodies3D.
            ...
            ref var list = ref bodies3D.ActiveSet.Constraints[originalIndex];
        ...
        /// <summary>
        /// Changes the body references of all constraints associated with two bodies3D in response to them swapping slots in memory.
        /// </summary>
        ...
        /// <summary>
        /// Enumerates the set of bodies3D associated with a constraint in order of their references within the constraint.
        /// </summary>
        /// <param name="constraintHandle">Constraint to enumerate.</param>
        /// <param name="enumerator">Enumerator to use.</param>
        ...
        public void EnsureSolverCapacities(int bodyHandleCapacity, int constraintHandleCapacity)
        {
            ...
            //Note that we can't shrink below the bodies3D handle capacity, since the handle distribution could be arbitrary.
            var targetBatchReferencedHandleSize = Math.Max(bodies3D.HandlePool.HighestPossiblyClaimedId + 1, bodyHandleCapacity);
            GetSynchronizedBatchCount(out var synchronizedBatchCount, out var fallbackExists);
