...
namespace Engine.Physics._3D.Collidables
{
    public abstract class ShapeBatch
    {
        ...
        /// <summary>
        /// Gets the number of physicsShapes3D that the batch can currently hold without resizing.
        /// </summary>
        ...
        {
            throw new InvalidOperationException("Nonconvex physicsShapes3D are not required to have a maximum radius or angular expansion implementation. This should only ever be called on convexes.");
        }
    ...
    public abstract class ShapeBatch<TShape> : ShapeBatch where TShape : unmanaged, IShape
    {
        ...
        //Note that physicsShapes3D cannot be moved; there is no reference to the collidables using them, so we can't correct their indices.
        //But that's fine- we never directly iterate over the physicsShapes3D set anyway.
        ...
        where TShapeWide : unmanaged, IShapeWide<TShape>
    {
        ...
        {
            //Most convex physicsShapes3D with an associated Wide type doesn't have any internal resources to dispose.
        }

        protected override void RemoveAndDisposeChildren(int index, PhysicsShapes3D physicsShapes3D, BufferPool pool)
        {
    ...
    {
        PhysicsShapes3D physicsShape3DBatches;

        public CompoundShapeBatch(BufferPool pool, int initialShapeCount, PhysicsShapes3D physicsShape3DBatches) : base(pool, initialShapeCount)
        {
            this.physicsShape3DBatches = physicsShape3DBatches;
            Compound = true;
        }
        ...
        protected override void RemoveAndDisposeChildren(int index, PhysicsShapes3D physicsShapes3D, BufferPool pool)
        {
            ...
            for (int i = 0; i < shape.ChildCount; ++i)
            {
                ...
                physicsShapes3D.RecursivelyRemoveAndDispose(child.ShapeIndex, pool);
            }
        }
        ...
        public override void ComputeBounds(int shapeIndex, in RigidPose pose, out Vector3 min, out Vector3 max)
        {
            shapes[shapeIndex].ComputeBounds(pose.Orientation, physicsShape3DBatches, out min, out max);
            min += pose.Position;
            max += pose.Position;
        }
        ...
        public override void RayTest<TRayHitHandler>(int shapeIndex, in RigidPose pose, in RayData ray, ref float maximumT, ref TRayHitHandler hitHandler)
        {
            shapes[shapeIndex].RayTest(pose, ray, ref maximumT, physicsShape3DBatches, ref hitHandler);
        }

        public override void RayTest<TRayHitHandler>(int shapeIndex, in RigidPose pose, ref RaySource rays, ref TRayHitHandler hitHandler)
        {
            shapes[shapeIndex].RayTest(pose, ref rays, physicsShape3DBatches, ref hitHandler);
        }
    }



    public class PhysicsShapes3D
    {
        ShapeBatch[] batches;
        ...
        public PhysicsShapes3D(BufferPool pool, int initialCapacityPerTypeBatch)
        {
        ...
        /// <summary>
        /// Removes a shape and any existing children from the physicsShapes3D collection and returns their resources to the given pool.
        /// </summary>
        ...
        /// <summary>
        /// Removes a shape from the physicsShapes3D collection and returns its resources to the given pool. Does not remove or dispose any children.
        /// </summary>
        /// <param name="shapeIndex">Index of the shape to remove.</param>
        /// <param name="pool">Pool to return all shape resources to.</param>
        ...
        /// <summary>
        /// Clears all physicsShapes3D from existing batches. Does not release any memory.
        /// </summary>
        public void Clear()
        {
        ...
        //PhysicsShapes3D, on the other hand, only get added or removed by the user.
        /// <summary>
        ...
        /// <summary>
        /// Releases all memory from existing batches. Leaves physicsShapes3D set in an unusable state.
        /// </summary>
        public void Dispose()
        {
            for (int i = 0; i < registeredTypeSpan; ++i)
