...
namespace Engine.Physics._3D
{
    ...
    public struct BoundingBoxBatcher
    {
        ...
        /// <summary>
        /// The number of bodies3D to accumulate per type before executing an AABB update. The more bodies3D per batch, the less virtual overhead and execution divergence.
        ...
        public unsafe void ExecuteConvexBatch<TShape, TShapeWide>(ConvexShapeBatch<TShape, TShapeWide> shapeBatch) where TShape : unmanaged, IConvexShape where TShapeWide : unmanaged, IShapeWide<TShape>
        {
            ...
            for (int bundleStartIndex = 0; bundleStartIndex < batch.Count; bundleStartIndex += Vector<float>.Count)
            {
                ...
                for (int innerIndex = 0; innerIndex < countInBundle; ++innerIndex)
                {
                    ...
                    //TODO: We bite the bullet on quite a bit of complexity to avoid merging on non-compounds. Could be better overall to simply merge on all bodies3D. Certainly simpler.
        ...
        public void Add(int bodyIndex, in RigidPose pose, in BodyVelocity velocity, in Collidable collidable)
        {
            ...
            //Technically, you could make a second pass that only processes collidables, rather than iterating over all bodies3D and doing last second branches.
            //But then you'd be evicting everything from cache L1/L2. And, 99.99% of the time, bodies3D are going to have physicsShapes3D, so this isn't going to be a difficult branch to predict.
