...
namespace Engine.Physics._3D.Collidables
{
    ...
    {
        int TypeId { get; }
        ShapeBatch CreateShapeBatch(BufferPool pool, int initialCapacity, PhysicsShapes3D physicsShape3DBatches);
    }

    //Note that the following bounds functions require only an orientation because the effect of the position on the bounding box is the same for all physicsShapes3D.
    ...
    //Note that we also support one-off bounds calculations. They are used even in the engine sometimes. Adding individual bodies3D to the simulation, for example.
    ...
    {
        //Note that compound physicsShapes3D have no wide GetBounds function. Compounds, by virtue of containing physicsShapes3D of different types, cannot be usefully vectorized over.
        //Instead, their children are added to other computation batches.
        void ComputeBounds(in Quaternion orientation, PhysicsShapes3D physicsShape3DBatches, out Vector3 min, out Vector3 max);
        void AddChildBoundsToBatcher(ref BoundingBoxBatcher batcher, in RigidPose pose, in BodyVelocity velocity, int bodyIndex);

        //Compound physicsShapes3D may require indirections into other shape batches. This isn't wonderfully fast, but this scalar path is designed more for convenience than performance anyway.
        //For performance, a batched and vectorized codepath should be used.
        void RayTest<TRayHitHandler>(in RigidPose pose, in RayData ray, ref float maximumT, PhysicsShapes3D physicsShape3DBatches, ref TRayHitHandler hitHandler) where TRayHitHandler : struct, IShapeRayHitHandler;
        void RayTest<TRayHitHandler>(in RigidPose pose, ref RaySource rays, PhysicsShapes3D physicsShape3DBatches, ref TRayHitHandler hitHandler) where TRayHitHandler : struct, IShapeRayHitHandler;
        int ChildCount { get; }
    ...
    /// </summary>
    /// <typeparam name="TChildShape">Type of the child physicsShapes3D.</typeparam>
    /// <typeparam name="TChildShapeWide">Type of the child physicsShapes3D, formatted in AOSOA layout.</typeparam>
