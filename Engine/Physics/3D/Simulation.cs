...
namespace Engine.Physics._3D
{
    ...
    public partial class Simulation : IDisposable
    {
        ...
        public Bodies3D Bodies3D { get; private set; }
        public Statics Statics { get; private set; }
        public PhysicsShapes3D PhysicsShapes3D { get; private set; }
        ...
        {
            var activeShapefulBodyCount = ValidateAndCountShapefulBodies(ref Bodies3D.ActiveSet, ref BroadPhase.ActiveTree, ref BroadPhase.activeLeaves);
            Debug.Assert(BroadPhase.ActiveTree.LeafCount == activeShapefulBodyCount);

            int inactiveShapefulBodyCount = 0;

            for (int setIndex = 1; setIndex < Bodies3D.Sets.Length; ++setIndex)
            {
                ref var set = ref Bodies3D.Sets[setIndex];
                if (set.Allocated)
                {
            ...
            for (int i = 0; i < Statics.Count; ++i)
            {
                ...
                Debug.Assert(collidable.Shape.Exists, "All static collidables must have physicsShapes3D. That's their only purpose.");
        ...
        /// <summary>
        /// Updates the position, velocity, world inertia, deactivation candidacy and bounding boxes of active bodies3D.
        /// </summary>
        ...
        /// <summary>
        /// Predicts the bounding boxes of active bodies3D by speculatively integrating velocity. Does not actually modify body velocities. Updates deactivation candidacy.
        /// </summary>
        /// <param name="dt">Duration of the time step.</param>
        /// <param name="threadDispatcher">Thread dispatcher to use for execution, if any.</param>
        ...
        /// <summary>
        /// Updates the velocities, world space inertias, bounding boxes, and deactivation candidacy of active bodies3D.
        /// </summary>
        /// <param name="dt">Duration of the time step.</param>
        /// <param name="threadDispatcher">Thread dispatcher to use for execution, if any.</param>
        ...
        /// <summary>
        /// Updates the velocities and world space inertias of active bodies3D.
        /// </summary>
        /// <param name="dt">Duration of the time step.</param>
        /// <param name="threadDispatcher">Thread dispatcher to use for execution, if any.</param>
        ...
        /// <summary>
        /// Updates the poses of active bodies3D.
        /// </summary>
        /// <param name="dt">Duration of the time step.</param>
        /// <param name="threadDispatcher">Thread dispatcher to use for execution, if any.</param>
        ...
        {
            //Note that constraint optimization should be performed after body optimization, since body optimization moves the bodies3D - and so affects the optimal constraint position.
        ...
        /// Other persistent allocations, like those in the Bodies3D set, will remain.
        /// </summary>
        public void Clear()
        {
            Solver.Clear();
        ...
        public void EnsureCapacity(SimulationAllocationSizes allocationTarget)
        {
            ...
            //Note that the bodies3D set has to come before the body layout optimizer; the body layout optimizer's sizes are dependent upon the bodies3D set.
            Bodies3D.EnsureCapacity(allocationTarget.Bodies);
            Bodies3D.MinimumConstraintCapacityPerBody = allocationTarget.ConstraintCountPerBodyEstimate;
            Bodies3D.EnsureConstraintListCapacities();
            Sleeper.EnsureSetsCapacity(allocationTarget.Islands + 1);
            Statics.EnsureCapacity(allocationTarget.Statics);
            PhysicsShapes3D.EnsureBatchCapacities(allocationTarget.ShapesPerType);
        ...
        public void Resize(SimulationAllocationSizes allocationTarget)
        {
            ...
            //Note that the bodies3D set has to come before the body layout optimizer; the body layout optimizer's sizes are dependent upon the bodies3D set.
            Bodies3D.Resize(allocationTarget.Bodies);
            Bodies3D.MinimumConstraintCapacityPerBody = allocationTarget.ConstraintCountPerBodyEstimate;
            Bodies3D.ResizeConstraintListCapacities();
            Sleeper.ResizeSetsCapacity(allocationTarget.Islands + 1);
            Statics.Resize(allocationTarget.Statics);
            PhysicsShapes3D.ResizeBatches(allocationTarget.ShapesPerType);
