...
namespace Engine.Physics._3D
{
        ...
        public unsafe bool TryAdd(int constraintHandle, int batchIndex, Solver solver, BufferPool pool, ref FallbackBatch fallbackBatch)
        {
            ...
            if (batchIndex == solver.FallbackBatchThreshold || ReferencedBodyIndices.CanFit(ref enumerator.BodyIndices[0], bodiesPerConstraint))
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < bodiesPerConstraint; ++i)
                    {
                        bodyHandles[i] = solver.bodies3D.ActiveSet.IndexToHandle[bodyIndices[i]];
                    }
                    fallbackBatch.AllocateForInactive(constraintHandle, ref *bodyHandles, bodiesPerConstraint, solver.bodies3D, constraintLocation.TypeId, pool);
                }
                return true;
            }
            return false;
    ...
    internal struct IslandScaffold
    {
        ...
        {
            Debug.Assert(bodyIndices.Count > 0, "Don't be tryin' to create islands with no bodies3D in them! That don't make no sense.");
