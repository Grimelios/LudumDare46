    ...
    public partial class Solver
    {
        ...
        struct IncrementalContactUpdateStageFunction : IStageFunction
        {
            ...
            public void Execute(Solver solver, int blockIndex)
            {
                ...
                solver.TypeProcessors[typeBatch.TypeId].IncrementallyUpdateContactData(ref typeBatch, solver.bodies3D, Dt, InverseDt, block.StartBundle, block.End);
            }
        }
            ...
            if (threadDispatcher == null)
            {
                ...
                for (int i = 0; i < activeSet.Batches.Count; ++i)
                {
                    ...
                    for (int j = 0; j < batch.TypeBatches.Count; ++j)
                    {
                        ...
                        {
                            TypeProcessors[typeBatch.TypeId].IncrementallyUpdateContactData(ref typeBatch, bodies3D, dt, inverseDt, 0, typeBatch.BundleCount);
                        }
                    }
                }
            }
