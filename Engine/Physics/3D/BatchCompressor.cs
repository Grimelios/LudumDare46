...
namespace Engine.Physics._3D
{
    ...
    public class BatchCompressor
    {
        ...
        //optimizer. The rebatching would have to take into account all the other bodies3D represented in the batches at each level. There is no guarantee this
        ...
        unsafe void DoJob(ref AnalysisRegion region, int workerIndex, BufferPool pool)
        {
            ...
            handleAccumulator.bodies3D = Bodies3D;
        ...
        private unsafe void ApplyCompression(int sourceBatchIndex, ref ConstraintBatch sourceBatch, ref Compression compression)
        {
            ...
            Solver.TypeProcessors[constraintLocation.TypeId].TransferConstraint(
                ref sourceBatch.GetTypeBatch(constraintLocation.TypeId), nextBatchIndex, constraintLocation.IndexInTypeBatch, Solver, Bodies3D, compression.TargetBatch);
        }
        ...
        public void Compress(BufferPool pool, IThreadDispatcher threadDispatcher = null, bool deterministic = false)
        {
            ...
            //does not affect any other optimization, because a ConstraintBatch guarantees that bodies3D are only referenced by a single constraint.
