...
namespace Engine.Physics._3D
{
    public interface IShapeRayHitHandler
    {
        ...
        /// <param name="childIndex">Index of the hit child. For convex physicsShapes3D or other types that don't have multiple children, this is always zero.</param>
    ...
    partial class Simulation
    {
        ...
        internal unsafe void GetPoseAndShape(CollidableReference reference, out RigidPose* pose, out TypedIndex shape)
        {
            ...
            {
                ref var location = ref Bodies3D.HandleToLocation[reference.Handle];
        ...
        struct RayHitDispatcher<TRayHitHandler> : IBroadPhaseRayTester where TRayHitHandler : IRayHitHandler
        {
            ...
            public unsafe void RayTest(CollidableReference collidable, RayData* rayData, float* maximumT)
            {
                if (ShapeHitHandler.HitHandler.AllowTest(collidable))
                {
                    ...
                    simulation.PhysicsShapes3D[shape.Type].RayTest(shape.Index, *pose, *rayData, ref *maximumT, ref ShapeHitHandler);
                }
            }
        }
        ...
        unsafe struct SweepHitDispatcher<TSweepHitHandler> : IBroadPhaseSweepTester, ISweepFilter where TSweepHitHandler : ISweepHitHandler
        {
            ...
            public unsafe void Test(CollidableReference reference, ref float maximumT)
            {
                if (HitHandler.AllowTest(reference))
                {
                    ...
                    simulation.PhysicsShapes3D[shape.Type].GetShapeData(shape.Index, out var targetShapeData, out _);
                    //Note that the velocity of the target shape is treated as zero for the purposes of a simulation wide cast.
                    //If you wanted to create a simulation velocity aware sweep, you would want to pull the velocity of the target from the Bodies3D set for collidable references
                    //that are associated with non-statics. It would look like this:
                    //BodyVelocity targetVelocity;
                    //if (reference.Mobility != CollidableMobility.Static)
                    //{
                    //    ref var location = ref Simulation.Bodies3D.HandleToLocation[reference.Handle];
                    ...
                    //        targetVelocity = Simulation.Bodies3D.ActiveSet.Velocities[location.Index];
                    //    else
                    ...
                    if (task != null)
                    {
                        var result = task.Sweep(
                            ...
                            ref this, simulation.PhysicsShapes3D, simulation.NarrowPhase.SweepTaskRegistry, Pool, out var t0, out var t1, out var hitLocation, out var hitNormal);
                        if (result)
                        {
                            if (t1 > 0)
                            {
