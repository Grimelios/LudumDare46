...
{
    /// <summary>
    /// Constrains two bodies3D to maintain a target twist angle around body-attached axes.
    /// </summary>
    ...
    public struct TwistServoFunctions : IConstraintFunctions<TwistServoPrestepData, TwistServoProjection, Vector<float>>
    {
        ...
        public static void ComputeJacobian(
            Bodies3D bodies3D, TwoBodyReferences bodyReferences, int count, in QuaternionWide localBasisA, in QuaternionWide localBasisB,
            out Vector3Wide basisBX, out Vector3Wide basisBZ, out Matrix3x3Wide basisA, out Vector3Wide jacobianA)
        {
            bodies3D.GatherOrientation(ref bodyReferences, count, out var orientationA, out var orientationB);

            //Twist joints attempt to match rotation around each body's local axis.
            //We'll use a basis attached to each of the two bodies3D.
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Prestep(Bodies3D bodies3D, ref TwoBodyReferences bodyReferences, int count, float dt, float inverseDt, ref BodyInertias inertiaA, ref BodyInertias inertiaB, ref TwistServoPrestepData prestep,
            out TwistServoProjection projection)
        {
            ComputeJacobian(bodies3D, bodyReferences, count, prestep.LocalBasisA, prestep.LocalBasisB,
