...
namespace Engine.Physics._3D.Constraints
{
    /// <summary>
    ...
    {
        void Prestep(Bodies3D bodies3D, ref TwoBodyReferences bodyReferences, int count, float dt, float inverseDt, ref BodyInertias inertiaA, ref BodyInertias inertiaB, ref TPrestepData prestepData, out TProjection projection);
        ...
        where TConstraintFunctions : unmanaged, IConstraintFunctions<TPrestepData, TProjection, TAccumulatedImpulse>
    {
        ...
        struct TwoBodySortKeyGenerator : ISortKeyGenerator<TwoBodyReferences>
        {
            ...
            public int GetSortKey(int constraintIndex, ref Buffer<TwoBodyReferences> bodyReferences)
            {
                ...
                //batching constraints which relate to wildly different bodies3D. Sorting by the minimum at least guarantees that two adjacent constraints will be as close as they can be
                //in one way. 
                ...
                //such a heuristic would need to score based on whether the bodies3D are in the same bundle. So, for example, two constraints get one 'close' point for each 
                //shared body bundle.
                ...
                //Even if you can't guarantee the next constraint is going to have bodies3D that are in cache, if you can generally lift the number of constraints
        ...
        public unsafe override void Prestep(ref TypeBatch typeBatch, Bodies3D bodies3D, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                bodies3D.GatherInertia(ref references, count, out var inertiaA, out var inertiaB);
                function.Prestep(bodies3D, ref references, count, dt, inverseDt, ref inertiaA, ref inertiaB, ref prestep, out projection);
            }
        }
        ...
        public unsafe override void WarmStart(ref TypeBatch typeBatch, ref Buffer<BodyVelocity> bodyVelocities, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                Bodies3D.GatherVelocities(ref bodyVelocities, ref bodyReferences, count, out var wsvA, out var wsvB); 
                function.WarmStart(ref wsvA, ref wsvB, ref projection, ref accumulatedImpulses);
                Bodies3D.ScatterVelocities(ref wsvA, ref wsvB, ref bodyVelocities, ref bodyReferences, count);
        ...
        public unsafe override void JacobiPrestep(ref TypeBatch typeBatch, Bodies3D bodies3D, ref FallbackBatch jacobiBatch, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ref var prestepBase = ref Unsafe.AsRef<TPrestepData>(typeBatch.PrestepData.Memory);
            ref var bodyReferencesBase = ref Unsafe.AsRef<TwoBodyReferences>(typeBatch.BodyReferences.Memory);
            ref var projectionBase = ref Unsafe.AsRef<TProjection>(typeBatch.Projection.Memory);
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                var count = GetCountInBundle(ref typeBatch, i);
                bodies3D.GatherInertia(ref references, count, out var inertiaA, out var inertiaB);
                //Jacobi batches split affected bodies3D into multiple pieces to guarantee convergence.
        ...
        public unsafe override void JacobiWarmStart(ref TypeBatch typeBatch, ref Buffer<BodyVelocity> bodyVelocities, ref FallbackTypeBatchResults jacobiResults, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                Bodies3D.GatherVelocities(ref bodyVelocities, ref bodyReferences, count, out wsvA, out wsvB);
                function.WarmStart(ref wsvA, ref wsvB, ref projection, ref accumulatedImpulses);
            }
        }
    ...
    {
        public unsafe override void IncrementallyUpdateContactData(ref TypeBatch typeBatch, Bodies3D bodies3D, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ref var prestepBase = ref Unsafe.AsRef<TPrestepData>(typeBatch.PrestepData.Memory);
            ref var bodyReferencesBase = ref Unsafe.AsRef<TwoBodyReferences>(typeBatch.BodyReferences.Memory);
            ref var bodyVelocities = ref bodies3D.ActiveSet.Velocities;
            var function = default(TConstraintFunctions);
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                ref var references = ref Unsafe.Add(ref bodyReferencesBase, i);
                var count = GetCountInBundle(ref typeBatch, i);
                Bodies3D.GatherVelocities(ref bodyVelocities, ref references, count, out var velocityA, out var velocityB);
