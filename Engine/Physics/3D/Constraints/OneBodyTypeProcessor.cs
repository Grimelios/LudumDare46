...
namespace Engine.Physics._3D.Constraints
{
    ...
    {
        void Prestep(Bodies3D bodies3D, ref Vector<int> bodyReferences, int count, float dt, float inverseDt, ref BodyInertias inertia, ref TPrestepData prestepData, out TProjection projection);
        ...
        where TConstraintFunctions : unmanaged, IOneBodyConstraintFunctions<TPrestepData, TProjection, TAccumulatedImpulse>
    {
        ...
        public unsafe override void Prestep(ref TypeBatch typeBatch, Bodies3D bodies3D, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                bodies3D.GatherInertia(ref references, count, out var inertiaA);
                function.Prestep(bodies3D, ref references, count, dt, inverseDt, ref inertiaA, ref prestep, out projection);
            }
        }
        ...
        public unsafe override void WarmStart(ref TypeBatch typeBatch, ref Buffer<BodyVelocity> bodyVelocities, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                Bodies3D.GatherVelocities(ref bodyVelocities, ref bodyReferences, count, out var wsvA);
                function.WarmStart(ref wsvA, ref projection, ref accumulatedImpulses);
                Bodies3D.ScatterVelocities(ref wsvA, ref bodyVelocities, ref bodyReferences, count);
            }
        }
        ...
        public unsafe override void JacobiPrestep(ref TypeBatch typeBatch, Bodies3D bodies3D, ref FallbackBatch jacobiBatch, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                ref var references = ref Unsafe.Add(ref bodyReferencesBase, i);
                var count = GetCountInBundle(ref typeBatch, i);
                bodies3D.GatherInertia(ref references, count, out var inertia);
                ...
                function.Prestep(bodies3D, ref references, count, dt, inverseDt, ref inertia, ref prestep, out projection);
            }
        }
        public unsafe override void JacobiWarmStart(ref TypeBatch typeBatch, ref Buffer<BodyVelocity> bodyVelocities, ref FallbackTypeBatchResults jacobiResults, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                Bodies3D.GatherVelocities(ref bodyVelocities, ref bodyReferences, count, out wsvA);
                function.WarmStart(ref wsvA, ref projection, ref accumulatedImpulses);
            }
        }
        ...
        where TConstraintFunctions : unmanaged, IOneBodyContactConstraintFunctions<TPrestepData, TProjection, TAccumulatedImpulse>
    {
        public unsafe override void IncrementallyUpdateContactData(ref TypeBatch typeBatch, Bodies3D bodies3D, float dt, float inverseDt, int startBundle, int exclusiveEndBundle)
        {
            ...
            for (int i = startBundle; i < exclusiveEndBundle; ++i)
            {
                ...
                ref var references = ref Unsafe.Add(ref bodyReferencesBase, i);
                var count = GetCountInBundle(ref typeBatch, i);
                Bodies3D.GatherVelocities(ref bodyVelocities, ref references, count, out var velocityA);
