...
namespace Engine.Physics._3D.Constraints
{
    ...
    public abstract class TypeProcessor
    {
        ...
        //N bodies3D, for N > Vector<float>.Count * 2. You may want to do SIMD internally in such a case, so there would be no 'bundles' at this level. Worry about that later.

        //We cache type id and bodies3D per constraint to avoid virtual calls.
        protected int typeId;
        protected int bodiesPerConstraint;
        public int TypeId { get { return typeId; } }
        /// <summary>
        /// Gets the number of bodies3D associated with each constraint in this type processor.
        /// </summary>
        ...
        /// <param name="bodyIndices">Pointer to a list of body indices (not handles!) with count equal to the type batch's expected number of involved bodies3D.</param>
    ...
    public abstract class TypeProcessor<TBodyReferences, TPrestepData, TProjection, TAccumulatedImpulse> : TypeProcessor where TBodyReferences : unmanaged where TPrestepData : unmanaged where TProjection : unmanaged where TAccumulatedImpulse : unmanaged
    {
        ...
        /// <param name="bodies3D">Bodies3D set that owns all the constraint's bodies3D.</param>
        /// <param name="targetBatchIndex">Index of the ConstraintBatch in the solver to copy the constraint into.</param>
        public unsafe override void TransferConstraint(ref TypeBatch typeBatch, int sourceBatchIndex, int indexInTypeBatch, Solver solver, Bodies3D bodies3D, int targetBatchIndex)
        {
            ...
            var bodyHandleCollector = new ActiveConstraintBodyHandleCollector(bodies3D, bodyHandles);
        ...
        internal unsafe sealed override void GatherActiveConstraints(Bodies3D bodies3D, Solver solver, ref QuickList<int> sourceHandles, int startIndex, int endIndex, ref TypeBatch targetTypeBatch)
        {
            ...
            for (int i = startIndex; i < endIndex; ++i)
            {
                ...
                //Body references get turned into handles so that awakening can easily track down the bodies3D' readded locations.
            ...
            int sourceStart, int targetStart, int count, Bodies3D bodies3D, Solver solver)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                for (int j = 0; j < bodiesPerConstraint; ++j)
                {
                    Unsafe.Add(ref targetReferencesLaneStart, offset) = bodies3D.HandleToLocation[Unsafe.Add(ref sourceReferencesLaneStart, offset)].Index;
                    offset += Vector<int>.Count;
                }
        ...
        internal unsafe sealed override void AddWakingBodyHandlesToBatchReferences(ref TypeBatch typeBatch, ref IndexSet targetBatchReferencedHandles)
        {
            for (int i = 0; i < typeBatch.ConstraintCount; ++i)
            {
                ...
                for (int j = 0; j < bodiesPerConstraint; ++j)
                {
                    ...
                    //Given that we're only adding references to bodies3D that already exist, and therefore were at some point in the active set, it should never be necessary
        ...
        internal override int GetBodyReferenceCount(ref TypeBatch typeBatch, int bodyToFind)
        {
            ...
            //Note that this function is used across both active and inactive sets. In the active set, the body references refer to *indices* in the Bodies3D.ActiveSet.
